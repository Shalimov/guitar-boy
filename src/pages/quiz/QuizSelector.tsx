...
export function QuizSelector({
  ...
  const [wizardDifficulty, setWizardDifficulty] =
  ...
  const QUIZ_PRESETS = QUICK_START_PRESETS.filter(
    (p) => p.id !== "custom" && !p.earTrainingMode,
  );
  const EAR_TRAINING_PRESETS = QUICK_START_PRESETS.filter(
    (p) => p.earTrainingMode,
  );
  ...
  if (!showWizard) {
    return (
      <div className="space-y-6">
        {/* Quiz Presets */}
        <div>
          <h3 className="text-lg font-semibold text-[var(--gb-text)] mb-4">
            Quick Start
          </h3>
      ...
      <div className="rounded-xl border border-[var(--gb-border)] bg-[var(--gb-bg-panel)] p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
          ...
          <div>
            ...
            <span className="font-medium text-[var(--gb-text)]">
              {wizardDifficulty.charAt(0).toUpperCase() +
                wizardDifficulty.slice(1)}
            </span>
          </div>
          <div>
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      color: active
                        ? "var(--gb-accent-strong)"
                        : "var(--gb-text)",
                    }}
                  >
                    {option.label}
                  </div>
                  <div className="mt-1 text-xs text-[var(--gb-text-muted)]">
                    {option.desc}
                  </div>
                </button>
              );
      ...
      {wizardStep === "difficulty" && (
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-[var(--gb-border)]">
            <p className="text-sm font-medium text-[var(--gb-text)] mb-3">
              Quiz Mode
            </p>
